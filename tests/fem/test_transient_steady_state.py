...
@pytest.mark.xfail(
    reason=(
        "M13.1 status (2026-04-26 follow-up #2): SG residual + analytic "
        "Jacobian correction are wired into the 1D transient runner via "
        "`semi.fem.sg_assembly.solve_sg_block_1d`. The analytic Jacobian "
        "correction in `assemble_sg_jacobian_correction_1d` is now "
        "FD-verified entry-by-entry against the residual `-sg_n - g_n` on "
        "a 3-node mesh at a non-equilibrium state — see "
        "`tests/fem/test_sg_assembly.py::"
        "test_assemble_sg_jacobian_correction_fd_verified` and "
        "`scripts/verify_sg_jacobian_fd.py`. Frobenius diff is "
        "~3e-9 relative across all 32 nonzero entries (no sign flip, "
        "no factor-of-2, no missing entry). The remaining blocker is "
        "POSITIVITY PRESERVATION, not a Jacobian bug: the discrete "
        "(n, p) primary-variable formulation lets a small numerical "
        "seed of negative `p` form in the depletion region at the "
        "first non-equilibrium timestep and double per step, "
        "diverging line-search around step 22-26. The recommended "
        "fix is a BC-ramp continuation in `run_transient` (see "
        "/tmp/m13.1-jac-fd-clean.md option 2)."
    ),
    strict=False,
)
