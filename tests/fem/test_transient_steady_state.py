...
@pytest.mark.xfail(
    reason=(
        "M13.1 status (2026-04-26 follow-up #3): BC-ramp continuation now "
        "ships in `run_transient` (see `_run_bc_continuation` and the "
        "`solver.bc_ramp_steps` field, default 10). Verified end-to-end: "
        "with continuation enabled, J at the time loop's t=0 matches "
        "`run_bias_sweep` at V_target to 5.7e-7 relative (J_t0 = 2.884213, "
        "J_ss = 2.884211). The IC is provably correct. The test still "
        "xfails because the (n, p) Galerkin time loop drifts to its OWN "
        "discrete fixed point within ~12 BDF steps and stays there: "
        "J_final = -6.273e+05 vs J_ss = +2.884. This is the architectural "
        "limitation already documented in `tests/mms/test_transient_"
        "convergence.py` (header, lines 14-17): the (n, p) Galerkin and "
        "Slotboom (phi_n, phi_p) discretisations have *different* discrete "
        "steady states (~3e18 m^-3 carrier-density mismatch at the "
        "depletion edge that does not vanish as dt -> 0). No IC choice "
        "can bridge that gap; the Galerkin time loop has its own fixed "
        "point and converges to it regardless. Closing this test requires "
        "switching the transient continuity discretisation from Galerkin "
        "to Scharfetter-Gummel (so the transient SS *is* the bias_sweep "
        "SS to spatial-discretisation precision). PR #44 ships the SG "
        "primitives + FD-verified analytic Jacobian (commit ab03b9f) and "
        "the BC-ramp infrastructure (this commit); wiring SG into the "
        "1D time-loop SNES function and Jacobian callbacks was attempted "
        "in follow-up #2 and regressed the M13 transient MMS test "
        "(line-search failure by step ~6). That regression's root cause "
        "was not fully nailed down before the cap was hit; the remaining "
        "work is M13.1 follow-up #4: re-attempt SG in the time loop with "
        "fresh diagnostics, and either fix the MMS regression or carry "
        "the SG path behind a `solver.use_sg_flux` flag with MMS pinned "
        "to Galerkin. See `/tmp/m13.1-integration-blocker.md` for the "
        "close-out audit."
    ),
    strict=False,
)
