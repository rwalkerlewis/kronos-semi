...
- **Practical examples.** See `examples/` for six
  self-contained device configs (NMOS / PMOS Id-Vgs, Schottky
  temperature dependence, power diode reverse recovery,
  MOSCAP C-V vs oxide thickness, pn diode reverse leakage
  temperature dependence) demonstrating M16 features in real
  engineering contexts. Distinct from `benchmarks/` (which
  gates V&V).
