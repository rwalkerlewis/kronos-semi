# Day 8 submission log

Day 8 is the final polish and submission-packaging pass on
`dev/day8-polish`, targeting PR #10 against `main` (PR number TBD; not
...
documentation drift from Days 1 through 7, regenerates the four
user-facing notebooks against the end-of-Day-7 capability surface,
...
to the post-Day-7 reality; rewriting the README status section as an
end-of-Day-7 capability matrix; regenerating `notebooks/01_pn_junction_1d.ipynb`
from the matching build script to strip Day-1 framing; authoring
...
`[0.8.0] - Day 8` entry; opening PR #10; and, post-merge under
...
surfaces a bug, it gets noted in the PR body and deferred to a Day 9
PR; the Day 8 branch does not grow to absorb it. This anti-scope-creep
rule is the submission-day failure mode being guarded against.

## Phase plan and execution

The Day 8 work is divided into eleven phases. Phase 0 is a baseline
...
**Phase 0 (baseline).** Before any Day 8 commit, the branch point
(`main` at `a604b12`, the Day 7 merge) was re-verified locally under
...
table were rewritten to mark Day 7 merged via PR #9 at `a604b12` and
to move Day 8 into "in flight" on `dev/day8-polish`. `docs/ROADMAP.md`
received the matching status ticks through Day 7. The Day 7 entry in
...
commits without re-reading the Day 7 PR.

**Phase 2 (`c7343c9`).** The README status section was rewritten from
a Day-1-centric pitch into a capability matrix listing every verified
...
rewritten to frame the notebook as the end-of-Day-7 intro rather than
a Day-1 artifact: references to later-shipped capabilities (bias
...
the values the Day-1 verifier produces against the shipped code.

**Phase 4 (`f7c83b0`).** `notebooks/02_pn_junction_bias.ipynb` was
authored against the Day 2-3 content: forward-bias Shockley sweep
...
against the Day 6 MOS C-V benchmark. The narrative carries the
disclosure that the verifier window is `[V_FB + 0.2, V_T - 0.1] V`
rather than the `V_FB + 0.1` edge the original Day 6 plan suggested;
...
against the Day 7 resistor benchmark. It runs the symmetric bipolar
...
across all four notebooks. `PLAN.md` picked up the "Day 9+ cleanups"
...
code; reconciling the derivation is deferred to Day 9+ to avoid
touching doctrinal physics text under submission-day time pressure.
The Day 6 benchmark and the Notebook 03 narrative both use the code's
convention, stated explicitly in both places.

### MOS verifier window V_FB+0.1 -> V_FB+0.2

The original Day 6 plan specified a verifier window of
...
### Bipolar sweep parameterization fix (Day 7 carryover)

`tests/test_bipolar_sweep.py` was added on the Day 7 merge and
...
pre-Day-7 count of 202 and corrected to 206 during the Phase 2
...
### Day 9+ cleanups (deferred)

The following pre-existing drift items were identified during the
Day 8 polish pass and are explicitly deferred to preserve PR #10
...
- `semi/__init__.py::__version__` still reads `"0.1.0"` from the Day 1
...
These items mirror the "Day 9+ cleanups" section already in `PLAN.md`
...
The Day 8 branch history was rebased locally before this log was
...
- `README.md` status section rewritten as the end-of-Day-7
  capability matrix.
- `PLAN.md` "Current state" updated and "Day 9+ cleanups" section
  added; this log is linked from a new "Day 8 execution" section
  near the top.
- `docs/ROADMAP.md` marked Day 7 complete and Day 8 in flight.
- `docs/day8-submission-log.md` (this file).

Pending in Phases 9-11 (not yet committed):

- `CHANGELOG.md` `[0.8.0] - Day 8` entry.
